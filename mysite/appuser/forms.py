...
class RegForm(forms.Form):
    ...
    verification_code = forms.CharField(
        label='验证码',
        required=False,
        widget=forms.TextInput(attrs={'class': 'form-control', 'placeholder': '点击发送验证码,发送到邮箱'})
    )
    ...
    def clean_verification_code(self):
        ...
        if not (code != '' and code == verification_code):
            raise forms.ValidationError('验证码不准确')
...
class ChangePersoncontentForm(forms.Form):
    ...
    def __init__(self, *args, **kwargs):
        if 'user' in kwargs:
            self.user = kwargs.pop('user')
        super(ChangePersoncontentForm, self).__init__(*args, **kwargs)
    ...
    def clean_Personcontent_new(self):
        personcontent_new = self.cleaned_data.get('text', '').strip()
        if personcontent_new == '':
            raise forms.ValidationError('内容不能为空')
        return personcontent_new

class ChangLocationForm(forms.Form):
    ...
    def __init__(self, *args, **kwargs):
        if 'user' in kwargs:
            self.user = kwargs.pop('user')
        super(ChangLocationForm, self).__init__(*args, **kwargs)
